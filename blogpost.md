...
As noted above, TabICL only leads TabPFN v3 nominally, 0.4269 against 0.4358 on the seed mean. However, TabICL is BSD 3-Clause, whereas TabPFN carries the Prior Labs Licence, Apache 2.0 with an added provision requiring "Built with PriorLabs-TabPFN" displayed on any related website, blog post or product documentation, and a TabPFN name prefix on models derived from it (as it applies to this very post). As the "Open" of OpenADMET signals, we prefer the more open license, all else equal. TabICL is not without its own downsides, though: it did not fit within our 24 GB GPU memory buffer, falling back to the central processing unit (CPU) at about 160 seconds per fit. TabPFN v3 runs in 14.6 seconds on the GPU for the same task. TabICL at 128 dimensions *does* fit, running 13 seconds per fit.
