...
TabICL's mean MAE runs nominally lower still on a leaner featureset: dropping the descriptor set entirely (CheMeleon embedding + log<sub>2</sub>FC readout only, 258 columns, the same "our best result overall" row shown above the dashed line in Figures 3 and 4) puts it at 0.4356, the best raw number anywhere in this project, though a paired test against the 386-column sweep can't distinguish the two at 5 seeds (*p* = 0.38). **Concatenation architecture** and **Best single-ingredient** are also repeated from prior sections for reference.
