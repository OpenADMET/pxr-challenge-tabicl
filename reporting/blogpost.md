...
We ran the challenge (as the OMSF contingent of OpenADMET) from April through July 2026, handing every team the same PXR induction assay data, holding out a 513-compound analog set, and scoring predicted pEC<sub>50</sub> against the unblinded truth. The CheMeleon baselines we ship in `openadmet-models` land around 0.51 to 0.52 MAE on that blind set (panel 00), well short of the roughly 0.41 the leading entries reached, so we were never going to top the leaderboard on our existing catalog alone.

We were not shopping for the single lowest score. A blind challenge rewards whatever squeezes out the last thousandth of an MAE, and the recipes near the top tend to be large ensembles tuned over many submissions. What we wanted was something we could fold into `openadmet-models` as a reproducible, YAML-defined pipeline: a method disclosed in enough detail to rebuild, and simple enough to keep running once the challenge fanfare faded. The fourth-place entry, for instance, blended nine members (five frozen graph encoders, two Boltz-2 structural models, and two tabular models) with a Caruana-weighted forward selection. That is a fine way to place in a challenge and a lot of moving parts to package, version, and rerun, for an edge smaller than the leaderboard's own bootstrap noise, which carried a standard deviation of roughly 0.02 MAE.
