...
Next we fixed the featureset and swapped only the tabular model. TabICL ran out of memory (OOM) on the 386-column embedding + readout + descriptors combination that Figure 3 crowned, both on the GPU (graphics processing unit) we used for that sweep and, when we reran the same fit on CPU (central processing unit) to check whether that was hardware-specific, there too: the process grew to about 30 GB of resident memory before the kernel's OOM-killer stopped it. Rather than read TabICL's row from a narrower featureset than its five rivals, we ran every regressor here, TabICL included, on the same 258-column CheMeleon embedding + readout inputs, no descriptors, so the comparison stays on equal footing. TabICL still came out ahead (0.4356 MAE), which is why we reported it as "our best overall" in previous sections. TabPFN v2.5 (0.4444 MAE) and TabPFN v3 (0.4453) were a near-dead heat behind it, a 0.0009 gap that is well within seed noise, with v2.6 next (0.4593). TabFM (0.4804 MAE) and the two gradient-boosted baselines (LightGBM 0.4977, XGBoost 0.5198) trailed all three TabPFN releases. We note that TabFM's number is not fully comparable: <code>max_num_rows</code> is capped at 500 in TabFM v1.0.0 because the full 4,139-row training set OOMs its attention pass on the GPU used here, so its in-context set is subsampled per ensemble member while TabPFN and TabICL took all 4,139 rows. Best GNN baseline and Best single-ingredient are repeated from prior sections for reference.

*(paste `reporting/figures/figure-03.html` here as an HTML card)*

> MAE by regressor on the shared featureset (CheMeleon embedding + readout, no descriptors, since TabICL OOMs on the 386-column combination the others could otherwise use): TabICL v2.1.1 0.4356 (best), then TabPFN v2.5 0.4444, TabPFN v3 0.4453, TabPFN v2.6 0.4593, TabFM 0.4804, LightGBM 0.4977, and XGBoost 0.5198.
